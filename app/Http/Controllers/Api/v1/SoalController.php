...
use App\Services\TestMateProcessDoc;
use App\Services\TestMateProcessHtml;
...
class SoalController extends Controller
{
    ...
    public function update(Request $request, Soal $soal)
    {
        ...
        try {
            ...
            $totalPilihan = count($request->pilihan);
            $c = $soal->daya_beda ?? 1; //Discrimination
            $a = $soal->tebak_parameter ?? 1; // Guessing
            $b = $soal->tingkat_kesulitan ?? 0.5; //Difficulty
    ...
    private function _formatTabled($request, $banksoal)
    {
        ...
        DB::beginTransaction();
        try {
            $testmateProc = new TestMateProcessDoc('1', $file, $dir);
            $data = $testmateProc->render();
            $questions = $data['data'];
            $files = $data['files'];
    ...
    private function _formatHTMLTable($request, $banksoal)
    {
        ...
        DB::beginTransaction();
        try {
            $testmateProc = new TestMateProcessHtml($file, $dir, $original_name);
            $data = $testmateProc->render();
