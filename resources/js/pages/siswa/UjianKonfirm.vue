    ...
    <div class="w-full lg:max-w-xl lg:py-4 lg:px-4 mb-20">
      <div class="bg-white border-gray-200 shadow sm:shadow-2xl py-2 rounded-t-xl rounded-b-xl">
        <div class="flex pt-2 pb-2">
          ...
          :class="tab == 0 ? 'border-darkblue' : 'border-gray-200'"
          @click="tab = 0"
          >
            <p class="font-medium text-gray-700 px-2">Test</p>
          </div>
          <div class="px-2 py-1 border-b-2 mb-2 cursor-pointer"
          :class="tab == 1 ? 'border-darkblue' : 'border-gray-200'"
          @click="tab = 1"
          >
        ...
        >
          ...
          <form @submit.prevent="ujianStart" v-if="jadwal && jadwal.length > 0">
            <template v-if="jadwalAllowEnter.length > 0">
              <div class="mb-4">
                <label for="" class="text-xs font-semibold text-gray-500 px-1">Jadwal Test</label>
                <div class="">
                  <div class="w-full">
                    <select v-model="data.jadwal_id" @change="checkToken()" class="text-gray-700 w-full pl-4 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-purple-300" :readonly="isLoading" :class="{'bg-gray-50' : isLoading}">
                      <option :value="jad.id" v-for="jad in jadwalAllowEnter" :key="jad.id">
                        {{ jad.alias  }} || {{ jad.tanggal }} || {{ jad.lama }} Menit
                      </option>
                    </select>
                  </div>
                </div>
              ...
              <!-- <div class="mb-4" v-if="active_token">
                ...
                <div class="flex">
                  <input type="text" v-model="data.token" class="w-full pl-4 pr-3 py-2 rounded-lg border-2 border-gray-200 outline-none focus:border-purple-300" placeholder="Token" required="" :readonly="isLoading" :class="{'bg-gray-50' : isLoading}">
                </div>
              </div> -->
              <div class="">
                <button type="submit" class="py-2 px-4 text-center bgdark text-white rounded-md hover:shadow-lg" :class="{ isLoading: 'bgdark' }">{{ isLoading ? 'Loading...' : 'Masuk' }}</button>
              </div>
            </template>
          </form>
          <template v-if="jadwal && jadwal.length > 0">
            <template v-if="jadwalNotAllowEnter.length">
              <div class="py-2 px-4 bg-purple-100 border border-gray-300 text-purple-600 rounded-md">
          ...
          <div v-else class="py-2 px-4 bg-yellow-100 border border-gray-300 text-yellow-600 rounded-md">
            Tidak ada test untuk anda saat ini
          </div>
			  </div>
        <div class="py-2 px-2 my-2 border-b border-gray-200"
        v-if="tab == 1"
          ...
          <table class="min-w-max w-full table-auto">
            <thead>
              <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
                <th class="py-3 px-6 text-left">Test Hari Ini</th>
