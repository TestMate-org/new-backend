 ...
 * @param {*} payload
 */
function FILLED_DATA_UJIAN_DETAIL(state, payload) {
    state.filledUjianDetail = payload
}
 ...
 * @param {*} store
 */
function takeFilledDetail({ commit }) {
    return new Promise(async (resolve, reject) => {
        try {
            commit('SET_LOADING',true, { root: true })
            const network = await $axios.get(endpoint.filled)
            commit('FILLED_DATA_UJIAN_DETAIL', network.detail)
            commit('SET_LOADING',false, { root: true })
            resolve(network.detail)
        } catch (error) {
            reject(getError(error))
            commit('SET_LOADING',false, { root: true })
        }
...
//  * @param {*} commit
//  */
// function getNextQuestion({ commit }, endpoint) {
//     return new Promise(async(resolve, reject) => {
//         try {
//             console.log(endpoint.nextQuestion)
//             let network = await $axios.post(endpoint.nextQuestion, payload)
//             resolve(network.data)
//         } catch (error) {
//             reject(getError(error))
//         }
 ...
 * @param {*} payload
 */
function getNextQuestion({ commit, state }, payload) {
    console.log(payload)
    commit('SET_LOADING', true, { root: true })
    return new Promise(async(resolve, reject) => {
        try {
            let network = await $axios.post('ujian/next-question', payload)
            commit('FILLED_DATA_UJIAN', network.data)
            commit('FILLED_DATA_UJIAN_DETAIL', network.detail)
            commit('SET_LOADING', false, { root: true })
            resolve(network.data)
